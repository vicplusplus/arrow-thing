...
- **`IGameMode`** (`Assets/Scripts/View/Scene/Modes/IGameMode.cs`) — interface that encapsulates per-mode behavior so `GameController` stays a thin orchestrator instead of accumulating `if (mode == ...)` branches. Surface: `string Name`, `IEnumerator Setup(GameContext)` (produces Board + view), `void Tick()` (per-frame mode logic), `void OnHudWired()` (mode-specific HUD tweaks), `Func<Cell, Vector3, bool> TapAttemptHandler`, `void OnArrowCleared()` (autosave / nothing), `void WireRunFlow()` (victory / topout / nothing), `bool SupportsSaveOnLeave`, `bool WouldOverwriteDifferentSave`, `bool HasInProgressChanges`, `Action OnQuickSaveHandler`, `void SaveAndLeave()`, `void Dispose()`. `GameContext` carries shared dependencies passed from controller to mode (read-only host services + writable Board/BoardView/etc that the mode populates during `Setup`). Implementations: **`ClassicMode`** (fixed-board solo runs — owns `GameTimer`, `GameTimerView`, `ReplayRecorder`, `VictoryController`, autosave + save/leave flow), **`EndlessMode`** (push-tick endless runs — owns `EndlessModeController`, `EndlessRun`, replay recording, topout result screen), **`CoopMode`** (live shared lobbies — owns `CoopClient`, `CoopSession`, `CoopPlayerTimer`, `CoopSidebar`, `CoopResultsScreen`, reconnect backoff). `GameController.CreateMode` selects an implementation based on `GameSettings`; new modes (PvP) drop in by adding the implementation + a case in `CreateMode`.
...
- **`LeaderboardScreenController`** — scene entry point for the Leaderboard scene. Manages 5 size tabs (Small/Medium/Large/XLarge/All), 3 sort buttons (Fastest/Biggest/Favorites), Local/Global scope toggle, scrollable entry list with context menu (delete, favorite toggle, replay launch — favorite and play are inline on wide screens, overflow to context menu on narrow/compact screens), and auto-scroll from victory screen via `GameSettings.LeaderboardFocusGameId`. Tab labels switch to abbreviated form (S/M/L/XL/All) on narrow viewports via `GeometryChangedEvent` to prevent the refresh button from clipping. On the All tab, Fastest is hidden (small boards always win) and Biggest is shown; on size tabs, the reverse. "Biggest" sort secondary tiebreaker: area → time → date. Top-3 entries in current sort receive gold/silver/bronze medal tints (suppressed in Favorites sort). Context menu flips above the row when it would overflow the bottom. Drag-to-scroll on content area. Context menu auto-dismisses on scroll. Global tab fetches top-50 entries and player entry from the server in parallel; stale results discarded if user switches away from Global view before response arrives. Player panel below the list shows rank context ("Your best: #N of T"), clickable login link (opens Settings), or descriptive error messages per HTTP status. Refresh button re-fetches. Global replays launch via `ApiClient.GetReplayAsync`. Compact mode (`lb-screen--compact`) activates below 500px panel width, hiding inline favorite/play buttons (available via context menu). Entry row icon buttons share a `.lb-row-btn` base class. Per-row UI construction is delegated to `LeaderboardEntryRow` (local view) and `LeaderboardGlobalEntryRow` (global view), the right-click / kebab menu to `LeaderboardContextMenu`, focus-nav graph wiring to `LeaderboardNavLinks`, time / date string formatting to `LeaderboardFormatters`, and gzip-base64 snapshot expansion on the global replay path to `LeaderboardReplayDecompress` (all under `Assets/Scripts/View/Scene/Leaderboard/`).
- **`LeaderboardEntryRow`** (`Assets/Scripts/View/Scene/Leaderboard/`) — per-row VisualElement builder. Construction takes a `Callbacks` bundle (`IsFavoritesSort`, `OnToggleFavorite`, `OnPlay`, `OnContextMenu`, `RegisterNameScroll`) so the row can delegate user interaction back to the controller without depending on its instance state. `Build(rank, entry, showSize, Medals)` returns the assembled row VisualElement; `Medals` is a small struct holding the gold/silver/bronze HashSet sets for the current render pass.
- **`LeaderboardContextMenu`** (`Assets/Scripts/View/Scene/Leaderboard/`) — the floating right-click / kebab menu sub-component. Owns the menu element, its delete-confirmation modal, the popup keyboard nav, and all per-action handlers. Construction takes a `Callbacks` bundle (`IsCompact`, `IsFavorite`, `OnToggleFavorite`, `OnPlay`, `OnDeleteConfirmed`); the controller drives the actual mutations (manager calls + post-rebuild refocus) on the `OnDeleteConfirmed` callback. Surface: `Show(gameId, isFavorite, anchorRow)`, `Dismiss()`, `IsOpen`, `IsKeyboardNavActive`, `UpdateKeyboardNav()`, `ContainsWorldPoint(pos)`. Position-flips above the row when it would overflow the bottom. `IsFavorite` lookup is re-read at delete-decision time so a favorite-toggle while the menu is open isn't acted on with stale state.
- **`LeaderboardNavLinks`** (`Assets/Scripts/View/Scene/Leaderboard/`) — stateless helper that wires the leaderboard's entire `FocusNavigator` graph after items have been added. `Apply(nav, items, Sections)` consumes a `Sections` bag of section start/end indices (header, mode tabs, size tabs, refresh, sort, entries, player panel) and sets up every `LinkBidi`/`Link`/`LinkBreak` edge — the top-row horizontal chain, the size-tab Up→active-mode-tab + Down→sort/entries fan-out, the entry-row × button-column grid, and the player-panel breakpoint links. Splitting "build items" and "wire links" into separate units keeps each phase independently readable. Focus restoration after rebuild stays on the controller (it inspects post-rebuild state).
- **`LeaderboardFormatters`** (`Assets/Scripts/View/Scene/Leaderboard/`) — static string helpers shared by the leaderboard scene and its row builder. `FormatTime` (h:mm:ss.fff / m:ss.fff / s.fff), `FormatCompactTime` (drops millis on All tab), `FormatRelativeDate` ("now", "5m ago", …, "1yr ago" relative to UTC now), `FormatExactDate` (local-time tooltip stamp).
- **`LeaderboardGlobalEntryRow`** (`Assets/Scripts/View/Scene/Leaderboard/`) — per-row VisualElement builder for the global leaderboard view. Parallel to `LeaderboardEntryRow` but a different shape: only a play button (no favorite or context-menu trigger) and the highlight tint marks the viewer's own entry rather than a favorite. Construction takes a `Callbacks` bundle (`OnPlay`, `RegisterNameScroll`); `Build(GlobalLeaderboardEntry, showSize, highlight)` returns the row VisualElement. `showSize` adds the size column and uses compact time format on the cross-config All tab.
- **`LeaderboardReplayDecompress`** (`Assets/Scripts/View/Scene/Leaderboard/`) — static helper that expands a gzip-base64 `boardSnapshot` field back into its nested-array form before `Newtonsoft.Json` deserializes a `ReplayData`. The server compresses the snapshot for top-50 rows; client deserializers expect the array shape. `DecompressSnapshotIfNeeded(string replayJson)` is a pure string transform — fail-soft: on any parse / decompression error, returns the original JSON unchanged and logs a warning so the downstream `ReplayData` deserialize surfaces the real error.
...
- **`BoardSetupHelper`** (static utility) — extracted shared logic from `GameController` for reuse in `ReplayViewController` and `CoopGameController`. Static methods: `CreateBoardAndView(width, height, visualSettings)` returns `(Board, BoardView)`; `SetupCamera(camera, board, zoomSpeed?)` returns `CameraController`; `RestoreBoardFromSnapshot(board, boardView, snapshot, frameBudgetMs)` coroutine restores arrows from `List<List<Cell>>` with frame-budget-aware yielding; `RestoreBoardFromSnapshotArrows(board, boardView, arrows, frameBudgetMs)` overload accepts `IReadOnlyList<Arrow>` directly (used by co-op).
...
- **`FocusNavigator`** — directed-graph keyboard focus navigation for UI Toolkit elements. Items are `FocusItem` structs with `Element`, `OnActivate`, `OnHorizontal`, `OnFocused`/`OnBlurred` callbacks, and a `CustomFocusVisual` flag. Navigation graph built with `Link`/`LinkBidi`/`LinkChain`/`LinkRow`/`LinkBreak`/`ClearLink`. `LinkBreak` edges only fire on initial key press (not DAS repeat) — used at region boundaries (e.g. last leaderboard entry → sort tabs). Visual ring via `kb-focusable`/`kb-focused` CSS classes. Modal push/pop stack for `ConfirmModal` overlays. `WasKeyboardActive` static flag carries ring state across scene transitions. Suppresses Unity's `NavigationMoveEvent`/`NavigationSubmitEvent`/`NavigationCancelEvent` in TrickleDown. Read-only `TryGetLink`/`IsLinkBreak` accessors expose the wired graph for tests and tools.
- **`NavGraph` + `NavGraphBuilder`** — declarative alternative for static screens. `NavGraph` is a `ScriptableObject` (assets under `Assets/Resources/NavGraphs/`) listing named slots, links (with `bidirectional` and `breakDAS` flags), and an optional `initialFocus`. `NavGraphBuilder` resolves slots to `VisualElement`s at runtime via `Bind(slotName, element, callbacks…)` and applies the result to a `FocusNavigator` with `Apply`. Slots whose element is null silently drop themselves and any link touching them, so a single SO can describe a graph that varies across platforms (e.g. Quit on desktop only). Used for the main menu root nav; dynamic graphs (preset grid, leaderboard rows, account-state-driven menus) keep using the imperative `Link*` API directly.
...
- **`BoardTrailRenderer`** — renders all arrow trails as a single combined mesh in the culling path. Each arrow occupies a 4-vertex slot; `AddArrow` appends, `RemoveArrow` uses swap-and-pop. Lazy upload via `RebuildIfDirty` (called from `LateUpdate`). The GameObject is inactive while trails are hidden; `BoardView.SetAllTrailsVisible` toggles it.
...
- **`LabeledField`** — labeled `TextField` wrapper with a bold label above the input. Used to standardize form fields in `AccountManager` (email, password, code, etc.).
...
## Co-op Server (`server/ArrowThing.Server/Coop/`)

- **`LobbyService`** — REST + DB access for lobbies. CRUD, status lookup, rate-limited create (5 per 30 min per user, global generation concurrency = 1), `MinBoardDimension = 20`, `MaxBoardDimension = 400` validation for client-supplied sizes.
- **`LobbyGenerationWorker`** — background service that picks up `Generating` lobbies, runs `BoardGeneration.Generate` with progress reporting, writes a `BinarySnapshot` blob + initial arrow list to the DB, transitions status to `Active`. Uses the shared `GenerationProgress` helper so client and server progress bars move in lockstep.
- **`CoopHub`** — WebSocket endpoint at `/ws/coop/{code}?token=...`. Per-connection state in `ConnectionEntry` (user id, token bucket, heartbeat/focus/input timestamps). Per-lobby hot state in `LobbyGameState` (live `Board`, seed, max-arrow-length, `NextSeq`, `SemaphoreSlim Mutex`, `LastActiveAt`, `EvictionCts`). `_gameStates` is a `ConcurrentDictionary<string, LobbyGameState>`.
  - **Hydration**: on first connect after eviction, `GetOrHydrateGameStateAsync` loads the snapshot + replays accepted `LobbyEvents` to rebuild the live `Board` and seed `NextSeq`.
  - **Atomic clear**: `HandleClearAttemptAsync` acquires the per-lobby mutex, token-bucket-checks the sender, re-reads the arrow at the tapped cell, calls `Board.IsClearable` / `Board.RemoveArrow`, persists an event row, and broadcasts `cleared` on accept. Rejects are all private to the sender (`rejected_dep` when the arrow isn't clearable, `rejected_race` when the cell is empty or out-of-bounds, `rejected_rate` when the token bucket is drained). `rejected_dep` exists purely as defense-in-depth: non-clearable taps are filtered client-side so the sender-correction path is rarely hit in practice. Last-arrow clears broadcast `lobby_completed` and transition `Lobbies.Status` to `Completed`.
  - **Rate limit**: per `(lobbyId, userId)` token bucket — 10 tokens capacity, 10/sec refill. Over-rate taps receive private `rejected_rate` with `retryAfterMs`; sustained over-rate for 5 s disconnects with `reason: "rate_limited"`.
  - **Heartbeat + watchdog**: clients send `heartbeat { focused, lastInputTsUtc }` every 15 s. Server stamps `LastHeartbeatAt` on the connection entry. A background `Timer` sweeps every 10 s and closes any socket whose last heartbeat is older than 30 s; the usual receive-loop unwind then queues the offline roster patch.
  - **Eviction**: 60 s grace after the last connection drops (`ScheduleGameStateEviction` → `Task.Delay` with a `CancellationTokenSource`). Any reconnect cancels the pending eviction and reuses the live state, preserving client visibility of in-flight clears across transient reconnects.
  - **Roster (Phase 7)**: on connect, `SendFullRosterAsync` ships a `roster_full` with every `LobbyRegistrations` row for the lobby. Changes (connect, disconnect, accepted clear bumping `ClearCount` + `FirstClearAt`, `timer_update` persisting `AccumulatedMillis`) dirty that user id in `LobbyRoom.PendingRoster`; a debounced flush (max one per 500 ms per lobby) broadcasts `roster_patch { upsert, remove }` with only the affected entries. Keeps payload size proportional to changes rather than total roster size, which matters once lobbies scale into the hundreds.
  - **Cleared payload**: includes the post-bump `newClearCount` + the clearer's `color` and `displayName` so other clients can update the sidebar and animate in-color without waiting for the next roster patch.
  - **Timer**: `HandleTimerUpdateAsync` validates `accumulatedMillis` is strictly greater than the stored value (ignores replays / rewinds), persists to `LobbyRegistrations.AccumulatedMillis`, queues a roster patch. Client-authoritative; server is just an echo point.
- **`LobbyReplayService`** (Phase 8) — builds a v6 `ReplayData` on demand from `LobbySnapshots.Data` (decoded via `BinarySnapshot.DecodeFull`) + `LobbyEvents` filtered to `ClearAccepted` (mapped to `clear` events with `playerId`) + `LobbyRegistrations` (projected into `roster`). `GET /api/lobbies/{code}/replay` authorizes registered participants only; 404 on missing/deleted, 410 on stripped snapshot. Serializes via `ReplayData.ToJson()` (Newtonsoft) to round-trip the public-field schema solo uses on disk.
- **`LobbyRetentionService`** (Phase 8) — `BackgroundService` with a 15-min initial delay + 24 h `PeriodicTimer` cadence. Runs two jobs sequentially each tick:
  - **SnapshotStripper**: finds `Completed`/`Deleted` lobbies older than 30 days with a non-null snapshot; nulls `LobbySnapshots.Data` and stamps `SnapshotStrippedAt`. Batched at 50 lobbies per pass.
  - **IdleReaper**: finds `Active`/`GenerationFailed` lobbies with `LastActivityAt < now - 30d`; transitions to `Deleted` with `DeletedAt = now`.
  Jobs no-op when nothing matches. Methods public for test access.
...
- **`SceneNavStackTests`** (EditMode) — unit tests for `SceneNavStack`, the pure stack logic that backs `SceneNav`. Models every real user flow through the scene graph (Play, Continue, Quick Reset, Victory → Menu, Victory → Leaderboard → Back, nested Replay paths) and verifies the correct scene is returned on each Pop.
