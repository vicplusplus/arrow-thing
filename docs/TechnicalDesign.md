...
- **`AuthService`** — all auth operations. Returns `(Response?, StatusCode, Error?)` tuples. Endpoints mapped in `Program.cs`. Methods: `RegisterAsync`, `LoginAsync`, `GetMeAsync`, `UpdateDisplayNameAsync`, `VerifyCodeAsync`, `ResendVerificationAsync`, `ForgotPasswordAsync`, `ResetPasswordAsync`, `ChangePasswordAsync`, `ChangeEmailAsync`, `ConfirmEmailChangeAsync`, `LockAccountAsync`, `UnlockAccountAsync`. All email flows use 6-digit codes entered in-app (no browser pages). `RegisterAsync` and `UpdateDisplayNameAsync` route the incoming display name through `DisplayNameValidator` and persist its trimmed output.
- **`DisplayNameValidator`** — static class. Validates display names at the two entry points where they enter the system (register, update). Enforces trimmed length 1-24, rejects Unicode control/format characters (including zero-width joiners and RTL overrides used for impersonation), and runs a profanity/slur substring check against a leet-folded, letters-only normalization of the input (so `sh1t`, `@sshole`, `n.i.g.g.e.r` are all caught). Banned token list is a short hard-coded set of slurs and common profanity — not exhaustive by design. `Validate(raw)` returns `(Ok, Error?, Normalized)`; callers persist `Normalized`.
