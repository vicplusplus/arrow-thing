...
- **`CoopHub`** — WebSocket endpoint at `/ws/coop/{code}?token=...`. Per-connection state in `ConnectionEntry` (user id, token bucket, heartbeat/focus/input timestamps). Per-lobby hot state in `LobbyGameState` (live `Board`, seed, max-arrow-length, `NextSeq`, `SemaphoreSlim Mutex`, `LastActiveAt`, `EvictionCts`). `_gameStates` is a `ConcurrentDictionary<string, LobbyGameState>`.
  ...
  - **Atomic clear**: `HandleClearAttemptAsync` acquires the per-lobby mutex, token-bucket-checks the sender, re-reads the arrow at the tapped cell, calls `Board.IsClearable` / `Board.RemoveArrow`, persists an event row, and broadcasts (`cleared` / `rejected_dep`) or privately replies (`rejected_race` / `rejected_rate`). Last-arrow clears broadcast `lobby_completed` and transition `Lobbies.Status` to `Completed`.
