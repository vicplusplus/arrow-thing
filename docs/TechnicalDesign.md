...
- Cross-runtime parity: `EndlessRun` uses `Math.Cos`, `Math.Pow`, `Math.Round`, and accumulated `float`/`double` arithmetic — primitives that are theoretically allowed to differ at the bit level between Mono (Unity editor + IL2CPP + WebGL) and .NET (server). `EndlessRunParityScenario` (`Assets/Scripts/Domain/`) drives a fixed-seed canonical run, FNV-1a-hashes every observable, and exposes `ExpectedHashV1`. Mirror tests (`Assets/Tests/EditMode/EndlessRunParityTests.cs` NUnit, `server/ArrowThing.Server.Tests/EndlessRunParityTests.cs` xUnit) assert both sides match the constant. **Procedure on tuning bumps**: bump `EndlessTuning.CurrentVersion`, run the server xUnit test (it'll fail and print the new hash), paste the value into `ExpectedHashV1`, then run the EditMode test in Unity to confirm Mono agrees. If Mono ever drifts, replace the offending `Math.*` call with a fixed-point or table-based equivalent — that's the failure case the parity tests exist to catch.
