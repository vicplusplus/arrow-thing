...
**Implemented.** Server-side: `Lobby` / `LobbyRegistration` / `LobbyEvent` / `LobbySnapshot` models + `User.CoopColor` column shipped via `AddCoopLobbies` migration. `Coop/LobbyService` (Create/Get/ListMine/SoftDelete with 5-active-lobby cap and code collision retry) and `Coop/CoopHub` (singleton WebSocket hub with hello/welcome/heartbeat/goodbye/echo handlers, JWT validation from query string, security stamp check). REST endpoints `POST /api/lobbies`, `GET /api/lobbies/me`, `GET /api/lobbies/{code}`, `DELETE /api/lobbies/{id}` and WebSocket endpoint `GET /ws/coop/{code}?token={jwt}` wired in `Program.cs`. Nginx `/ws/coop/` location with upgrade headers + `access_log off` (JWT in query string). Serilog filters `/ws/coop/` request logs to verbose for the same reason. 18 new server tests pass (12 lobby + 6 hub) on top of the existing 110. Client-side: `ApiClient` lobby methods, cross-platform `CoopClient` (desktop = `System.Net.WebSockets.ClientWebSocket`, WebGL = `.jslib` bridge to browser `WebSocket` via `CoopWebGLBridge` MonoBehaviour), `Tools/Arrow Thing/Create Test Lobby` editor menu. Lobby code alphabet is letters-only (`ABCDHJLPQRSTUVWXYZ`, 18 chars, ~34M combinations) — excludes `I/O` (ambiguous with `1/0`) and `E/F/G/K/M/N` (commonly appear in slurs); no digits to avoid culturally loaded numbers.
...
**Implemented.** New `BinarySnapshot` codec (in `Assets/Scripts/Domain/BinarySnapshot.cs`, shared with the server via the existing wildcard include) with two layers: `EncodeArrows`/`DecodeArrows` (no header — used by replays) and `EncodeFull`/`DecodeFull` (32-byte header per the roadmap spec — used for the co-op WebSocket transport, with optional gzip). **Replay schema bumped to v5**: `ReplayData.boardSnapshot` is now a base64-encoded `BinarySnapshot` blob instead of a `List<List<Cell>>`. A custom Newtonsoft converter on the field reads both v4 (legacy cell-array JSON) and v5 (string) transparently, so existing local saves keep working. `ReplayVerifier`, `ReplayPlayer`, `GameController`, `ReplayViewController`, and `LeaderboardScreenController` all use the new `GetSnapshotArrows()` / `SetSnapshotArrows()` helpers. Server side: `LobbyGenerationWorker` (`BackgroundService` registered in `ArrowThing.Worker`, also in-process for tests via `TestFactory`) consumes the `coop:gen:queue` Redis list, runs `BoardGeneration.FillBoardIncremental` under an `AccountConcurrencyLimiter` (3 global / 1 per account, per process), persists the snapshot via `LobbySnapshotRepository`, and publishes progress / completion / failure events through a Redis pub/sub `GenerationProgressBus` (`coop:lobby:{code}` channels). `CoopHub` subscribes to the bus on startup and broadcasts events to connected sockets in the matching room. `LobbyService.CreateAsync` enqueues a generation job after persisting the lobby; `LobbyService.RetryGenerationAsync` (POST `/api/lobbies/{id}/retry-gen`, owner-only) re-enqueues failed lobbies. `CoopHub.HandleHelloAsync` dispatches by current lobby status: `Generating` → wait for bus events; `Active` → send `welcome` + `snapshot` JSON envelope + binary frame loaded from `LobbySnapshotRepository`; `Completed`/`GenerationFailed`/`Deleted` → send `disconnect` and close. The Unity `CoopClient` gained a `BinaryReceived` event and binary frame handling on both desktop (`ClientWebSocket.ReceiveAsync` returns `WebSocketMessageType.Binary`) and WebGL (`.jslib` bridge sets `binaryType='arraybuffer'`, base64-encodes binary frames over `SendMessage`, decoded back via `CoopClient.OnJsBinary`). `LobbyOptions` config record (`DefaultWidth`/`DefaultHeight`/`MaxArrowLength`) is bound via `IOptions<LobbyOptions>` so tests can override the default 200×200 board to 20×20 for fast generation. 5 new server integration tests (full create→generate→connect→snapshot flow, retry-gen happy path, retry-gen 403, retry-gen 400) plus 12 new domain tests (`BinarySnapshotTests`, `ReplayDataMigrationTests`) on top of the existing 128 — **133 server tests total, all passing**.
...
**Implemented.** Hub layout defaults to My Lobbies list with Host and Join as modal overlays (not the originally proposed three-panel tab layout). Coop color uses a dedicated `PATCH /api/auth/me/coop-color` endpoint (not the existing display-name PATCH). Default color: fixed 12-color palette hashed by user id (`CoopColorPalette`, shared between server and client). No contrast/colorblind indicators — descoped from Phase 5. Deep-link: WebGL reads `window.location.search` via `CoopDeepLink.jslib`; Desktop reads `--lobby=XXXXXX` from CLI args. New `KeybindManager.Context.CoopHub` enum entry. Mobile narrow-layout built (breakpoint 768px). Server-side: `LobbyService.CreateAsync` auto-registers the creator; `ListMineAsync` broadened to owned-or-registered lobbies with per-row owner display name, `youAreOwner`, and `yourClearCount` fields; `CoopHub.HandleHelloAsync` upserts `LobbyRegistration` with 50-per-user active cap. Co-op mode integrated into `GameController` (not a separate scene) — when `GameSettings.ActiveLobbyCode` is set, connects `CoopClient`, receives snapshot, renders board read-only via `BinarySnapshot.DecodeFull` using the existing camera/input/HUD infrastructure. No clear attempts (Phase 6). `GameSettings.PendingLobbyCode` / `ActiveLobbyCode` with consume helpers for deep-link → hub → game scene handoff. 8 new server tests (141 total). New `SceneNavStackTests` for co-op scene transitions. `CoopHubLayoutTests` covering 4 hub states × 5 aspect ratios. Scene files (`CoopHub.unity`, `CoopGame.unity`) require manual Unity Editor setup.
