...
/// <summary>
/// All endless-mode game-loop constants in one struct so the client
/// (<see cref="EndlessModeController"/>) and the server-side replay
...
/// </summary>
public readonly struct EndlessTuning
{
    ...
    /// </summary>
    public static readonly EndlessTuning V1 = new EndlessTuning(
    ...
    /// <summary>
    /// Picks the historical tuning matching <paramref name="version"/>.
    /// Throws on unknown versions — submission pre-verify gates this so
    /// the worker doesn't see unsupported versions.
    /// </summary>
    public static EndlessTuning ForVersion(int version)
    {
        return version switch
        {
            1 => V1,
            _ => throw new ArgumentException(
                $"Unknown endless tunings version: {version}",
                nameof(version)
            ),
        };
    }

    /// <summary>Highest tuning version currently produced by clients.</summary>
